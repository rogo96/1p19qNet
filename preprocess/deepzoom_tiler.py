    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                # tile processing  
                ###########################################
                if edge > self._threshold :    # default = 15,  filtering background
                    # not enough tile's size --> resize
                    # print('usefule tile save')
                    if not (w==self._tile_size and h==self._tile_size):
                        tile = tile.resize((self._tile_size, self._tile_size))
                    tile.save(outfile, quality=self._quality)
                    self.filter_blood(outfile)
            except:
                pass
            self._queue.task_done()
...
class DeepZoomImageTiler(object):
    ...
    def __init__(self, dz, basename, target_levels, mag_base, format, associated, queue, slide_dir):
        self._dz = dz
        ...
        for level in range(self._dz.level_count):
            ...
            for row in range(rows):
                for col in range(cols):
                    tilename = os.path.join(tiledir, self.slide_dir + '_' +
                                            '%d_%d.%s' % (
